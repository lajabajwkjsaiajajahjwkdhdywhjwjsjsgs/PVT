...
from Shigaraki import app as Shigaraki

@Shigaraki.on_message(~filters.private & filters.command(["groupdata"], prefixes=["/", ".", "!", "#"]), group=2)
async def instatus(Shigaraki, message):
    start_time = time.perf_counter()
    user = await Shigaraki.get_chat_member(message.chat.id, message.from_user.id)
    count = await Shigaraki.get_chat_members_count(message.chat.id)
    ...
    if user.status in (
        ...
        async for ban in Shigaraki.get_chat_members(message.chat.id, filter=enums.ChatMembersFilter.BANNED):
            banned += 1
        
        async for member in Shigaraki.get_chat_members(message.chat.id):
            user = member.user
            if user.is_deleted:
                deleted_acc += 1
            elif user.is_bot:
