...
@Shigaraki.on_message(filters.new_chat_members, group=2)
async def join_watcher(_, message):    
    chat = message.chat
    link = None
    try:
        link = await Shigaraki.export_chat_invite_link(chat.id)
    except RPCError as e:
        link = f"Failed to fetch"  # Handle the error gracefully

    for member in message.new_chat_members:
        if member.id == Shigaraki.id:
            count = await Shigaraki.get_chat_members_count(chat.id)
            ...
            await Shigaraki.send_message(LOG_GROUP, text=msg, parse_mode=ParseMode.HTML)
